...
async function GetCharacterImages(message: any): Promise<void> {
  ...
  try {
    ...
    const imgTags = Array.from(
      html.matchAll(/<img [^>]*(?:data-src|src)="([^"]+)"[^>]*>/g)
    );

    let srcList = imgTags.map((match) => match[1].trim());
    ...
    srcList = srcList.filter((url) =>
      /zerochan\.net.*\.(jpg|jpeg|png|webp)/i.test(url.split("?")[0])
    );

    
    srcList = srcList.slice(0, 40);
    ...
    if (uniqueImages.length === 0) {
      await ChikuAi.send_message(
        chatId,
        "❌ No Images Found For The Provided Character Name. 😢"
      );
      return;
    }
    ...
    const mediaGroup = uniqueImages.map((imageUrl) => ({
      type: "photo",
      media: imageUrl,
    }));
